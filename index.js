...
const { connectToDatabase, getDataFromId, getDataFromBuildingName, getDataFromName, getDataFromLocality, getRandomDataFromDessert, 
  getRandomDataFromGrilled_shabu, getRandomDataFromMuslim, getRandomDataFromNoodles, getRandomDataFromPharmacy, getRandomDataFromRestaurant, getDataFromRoom} = require('./database');
...
//ร้านอาหารตามสั่ง + โลเคชั่น
  ...
  textResponse = `ของแนะนำ: ร้าน ${data.name} ที่ตั้ง: ${data.map_link} ครับ`;
...
//สถานที่
  ...
  const id = 8;
  data = await getDataFromLocality(connection, id);
  textResponse = `อุทยานบัว ที่อยู่ : ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
...
//ห้อง
} else if (intent === 'room1') {
  const id = 1;
  data = await getDataFromRoom(connection, id);
  textResponse = `นี่คือปฏิบัติการวิศวกรรมไฟฟ้า4 ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room2') {
  const id = 2;
  data = await getDataFromRoom(connection, id);
  textResponse = `ปฏิบัติการวิศวกรรมไฟฟ้า2 ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room3') {
  const id = 3;
  data = await getDataFromRoom(connection, id);
  textResponse = `ปฏิบัติการวิศวกรรมไฟฟ้า3 ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room4') {
  const id = 4;
  data = await getDataFromRoom(connection, id);
  textResponse = `ฝึกงาน-สหกิจ ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room5') {
  const id = 5;
  data = await getDataFromRoom(connection, id);
  textResponse = `งานวิชาการ-การเรียนการสอน ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room6') {
  const id = 6;
  data = await getDataFromRoom(connection, id);
  textResponse = `ห้องสาขาวิศวกรรมไฟฟ้าและคอมพิวเตอร์ ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room7') {
  const id = 7;
  data = await getDataFromRoom(connection, id);
  textResponse = `ปฏิบัติการจุลชีววิทยา ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room8') {
  const id = 8;
  data = await getDataFromRoom(connection, id);
  textResponse = `ห้องพรรณานิคม ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room9') {
  const id = 9;
  data = await getDataFromRoom(connection, id);
  textResponse = `ห้องกุสุมาลย์ ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room10') {
  const id = 10;
  data = await getDataFromRoom(connection, id);
  textResponse = `ห้องสว่างแดนดิน ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room11') {
  const id = 11;
  data = await getDataFromRoom(connection, id);
  textResponse = `ห้องแลปวิทยาศาสตร์ ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room12') {
  const id = 12;
  data = await getDataFromRoom(connection, id);
  textResponse = `ห้องระพี สาคริก ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room13') {
  const id = 13;
  data = await getDataFromRoom(connection, id);
  textResponse = `งานทะเบียนและประมวลผล ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room14') {
  const id = 14;
  data = await getDataFromRoom(connection, id);
  textResponse = `งานเทคโนโลยีสารสนเทศ ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room15') {
  const id = 15;
  data = await getDataFromRoom(connection, id);
  textResponse = `งานกิจการนิสิต กองบริหารวิชาการและนิสิต ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room16') {
  const id = 16;
  data = await getDataFromRoom(connection, id);
  textResponse = `หน่วยแนะแนวและรับเข้า ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else if (intent === 'room17') {
  const id = 17;
  data = await getDataFromRoom(connection, id);
  textResponse = `งานกิจการนิสิต แนะแนวทุนการศึกษา ${data.map_link} รูปภาพ: ${data.image_file} ครับ`;
} else {
  textResponse = 'สอบถามเรื่องอะไรครับ';
}
...
app.post('/', async (req, res) => {
  ...
  try {
    ...
    // เรียกใช้ฟังก์ชัน addConversationHistory และใส่ค่า responseText ที่เป็นคำตอบของแชทบอท
    ...
    if (
      ...
      data = await getDataFromBuildingName(connection); //หมายเลขอาคาร
    } else if (
      ...
      data = await getRandomDataFromMuslim(connection); //สุ่มร้านฮาลาลน
    } else if (intent === 'noodles') {
      data = await getRandomDataFromNoodles(connection); //สุ่มร้านก๋วยเตี๋ยว
    } else if (intent === 'pharmacy') {
      data = await getRandomDataFromPharmacy(connection); //สุ่มร้านขายยาว
    }  else if (
      intent === 'room1'  || intent === 'room2'  ||
      intent === 'room3'  || intent === 'room4'  ||
      intent === 'room5'  || intent === 'room6'  ||
      intent === 'room7'  || intent === 'room8'  ||
      intent === 'room9'  || intent === 'room10' ||
      intent === 'room11'  || intent === 'room12' ||
      intent === 'room13'  || intent === 'room14' ||
      intent === 'room15'  || intent === 'room17' 
      ) {
      data = await getDataFromRoom(connection); //ห้อง
    }
